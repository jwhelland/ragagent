...
class ResearchAgent:
    ...
    def research(
        ...
        if status_callback:
            status_callback("Starting deep research...")
        ...
        for i in range(1, max_iterations + 1):
            if status_callback:
                status_callback(
                    f"Research iteration {i}/{max_iterations}: Assessing sufficiency..."
                )
            ...
            if status_callback:
                status_callback(
                    f"Gap detected: {sufficiency.missing_information[0] if sufficiency.missing_information else 'Unknown'}"
                )
            ...
            for idx, sub_q in enumerate(sub_queries, 1):
                ...
                try:
                    ...
                    sub_result = self.retriever.retrieve(sub_parsed, top_k=5, generate_answer=False)
    ...
    def _assess_sufficiency(self, query: str, context: List[HybridChunk]) -> SufficiencyResult:
        ...
        # Prepare Prompt
        system_template = self.response_generator.prompts.get("sufficiency_check", {}).get(
            "system", ""
        )
        user_template = self.response_generator.prompts.get("sufficiency_check", {}).get(
            "user_template", ""
        )

        user_prompt = user_template.format(
            query_text=query, context_summary=context_summary or "No context gathered yet."
        )
    ...
    def _generate_sub_queries(self, original_query: str, missing_info: List[str]) -> List[str]:
        ...
        system_template = self.response_generator.prompts.get("sub_query_generation", {}).get(
            "system", ""
        )
        user_template = self.response_generator.prompts.get("sub_query_generation", {}).get(
            "user_template", ""
        )
    ...
    def _synthesize_final_answer(
        self, query: str, chunks: List[HybridChunk], query_id: str
    ) -> GeneratedResponse:
        ...
        sorted_chunks = sorted(
            chunks, key=lambda c: c.final_score if c.final_score is not None else 0.0, reverse=True
        )
        ...
        return self.response_generator.generate(
            query_text=query, retrieval_result=result, prompt_key="synthesis"
        )

    def _deduplicate_chunks(
        self, chunks: List[HybridChunk], seen_ids: Set[str]
    ) -> List[HybridChunk]:
