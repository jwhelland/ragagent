...
class GraphStore:
    ...
    def upsert_document(self, doc_id: str, sha256: str, path: str) -> None:
        cypher = (
            "MERGE (d:Document {id: $id}) SET d.sha256=$sha, d.path=$path RETURN d"
        )
        self._run(cypher, {"id": doc_id, "sha": sha256, "path": path})

    def upsert_section(self, doc_id: str, page: int, chunk_id: str) -> None:
        cypher = (
            "MERGE (s:Section {chunk_id: $chunk_id}) SET s.document_id=$doc_id, s.page=$page RETURN s"
        )
    ...
    def add_episode_for_chunk(self, *, name: str, body: str, doc_id: str, page: int | None, source_desc: str = "PDF chunk") -> None:
        return

    def get_related_sections(self, chunk_ids: List[str], limit_per_seed: int = 2) -> List[Dict[str, Any]]:
        if not chunk_ids:
            return []
    ...
    def search_sections_by_entities(self, entities: List[str], limit: int = 5) -> List[Dict[str, Any]]:
        if not entities:
            return []
        cypher = """
