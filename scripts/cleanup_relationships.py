...
def promote_valid(
    ...
    norm_table = NormalizationTable(
        table_path=config.normalization.normalization_table_path, config=config.normalization
    )
    ...
    try:
        ...
        while True:
            candidates = manager.get_relationship_candidates(
                status=CandidateStatus.PENDING.value, limit=batch_size, offset=offset
            )
            if not candidates:
                break
            ...
            for cand_dict in candidates:
                ...
                is_valid = (
                    source_norm
                    and source_norm.status == EntityStatus.APPROVED
                    and target_norm
                    and target_norm.status == EntityStatus.APPROVED
                )

                if is_valid:
                    if dry_run:
                        console.print(
                            f"[dim]Would promote:[/dim] {cand_dict['source']} -> {cand_dict['target']}"
                        )
                    else:
...
def prune_weak(
    ...
    service = EntityCurationService(
        manager,
        NormalizationTable(table_path=config.normalization.normalization_table_path),
        config,
    )
    ...
    try:
        ...
        processed_count = 0
        while True:
            results = manager.execute_cypher(
                query,
                {
            ...
            if dry_run:
                console.print(
                    f"Would reject {len(candidates)} candidates (e.g. {candidates[0]['source']} -> {candidates[0]['target']} [{candidates[0]['confidence_score']}])"
                )
                break
